...
namespace TvShowAPI.Controllers
{
    ...
    public class SeriesController : ControllerBase
    {
        ...
        [HttpGet]
        public IActionResult GetSeries(int? pageNumber, int? pageSize)
        {
            int currentPageNumber = pageNumber ?? 1;
            int currentPageSize = pageSize ?? 10;

            var series = _dbContext.Series.Include(a => a.ActorSeries).ThenInclude(a => a.Actor); ;
            return Ok(series.Skip((currentPageNumber - 1) * currentPageSize).Take(currentPageSize));
        }

        [HttpGet("[action]")]
        public IActionResult MoreDetails(int serieID, int? pageNumber, int? pageSize)
        {
            int currentPageNumber = pageNumber ?? 1;
            int currentPageSize = pageSize ?? 10;
        ...
        [HttpGet("[action]")]
        public IActionResult SeriesActors(int serieID, int? pageNumber, int? pageSize)
        {
            int currentPageNumber = pageNumber ?? 1;
            int currentPageSize = pageSize ?? 10;
        ...
        [HttpGet("[action]")]
        public IActionResult SerieGenre(string genre, int? pageNumber, int? pageSize)
        {
            int currentPageNumber = pageNumber ?? 1;
            int currentPageSize = pageSize ?? 10;
